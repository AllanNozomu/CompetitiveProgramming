...
int get_sum(const vector<int> &st, int ss, int se, int qs, int qe, int si){
    ...
    return get_sum(st, ss, MID, qs, qe, LEFT) + 
            get_sum(st, MID + 1, se, qs, qe, RIGHT);
}

int get_sum(const vector<int> &st, int n, int qs, int qe){
    return get_sum(st, 0, n - 1, qs, qe, 0);
}

void update_range(vector<int> &st, int ss, int se, int us, int uf, int val, int si){
    ...
    if (IS_LEAF){                          // Update usign bottom-up
        ...
        update_range(st, ss, MID, us, uf, val, LEFT);
        update_range(st, MID + 1, se, us, uf, val, RIGHT);

        st[si] = st[LEFT] + st[RIGHT];
    }
}

void update_range(vector<int> &st, int n, int us, int uf, int val){
    update_range(st, 0, n - 1, us, uf, val, 0);
}
...
void construct_segtree(vector<int> &st, const vector<int> &arr){
...
int main()
{
    ...
    cin >> i >> j;
    while(i >= 0 && j >= 0){
        cout << get_sum(st, input.size(), i, j) << endl;
        cin >> i >> j;
    }
